<div class="w-full flex flex-col justify-start items-start">
    ...
    @else
        <div class="w-full flex flex-row justify-start items-center gap-5 updating-section">
            ...
            @endif
            @if((has_all_permissions(auth()->user(), 'update-all-tickets') || (has_all_permissions(auth()->user()->id, 'update-own-tickets') && ($ticket->owner_id === auth()->user() || $ticket->responsible_id === auth()->user()->id))) && has_all_permissions(auth()->user(), 'assign-tickets'))
