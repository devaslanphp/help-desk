        ...
        <div class="w-full flex flex-row gap-2 px-5 border-b border-gray-200 pb-5 mb-5">
            ...
            <div class="flex flex-col justify-start items-start gap-3">
                <div class="flex flex-col justify-center items-start gap-0">
                    ...
                    <span
                        class="text-gray-500 text-xs font-light">@lang('Added a comment') {{ $comment->created_at->diffForHumans() }}</span>
                </div>
                    ...
                    @if($updating && $comment->id === $selectedComment->id)
                        <form wire:submit.prevent="save" class="w-full relative flex flex-col justify-start items-start gap-2">
                            ...
                            <div class="flex flex-row items-center gap-1">
                                <button type="submit"
                                        class="py-2 px-3 rounded-lg shadow hover:shadow-lg bg-primary-700 hover:bg-primary-800 text-white text-base flex flex-row gap-2 items-center">
                    ...
                    @if($comment->owner_id === auth()->user()->id && (!$updating || $comment->id !== $selectedComment?->id))
                        <div class="w-full flex flex-row justify-start items-center gap-5">
                            ...
                            <button {!! $deleteConfirmationOpened ? 'disabled' : '' !!} type="button"
                                    ...
                                    class="text-gray-400 text-xs hover:text-danger-500 flex flex-row justify-start items-center gap-1">
