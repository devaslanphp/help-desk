                    ...
                    @if($types->count())
                        @foreach($types as $type)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="py-4 px-6">
                                    <span class="px-3 py-1 rounded-full text-sm" style="color: {{ $type->text_color }}; background-color: {{ $type->bg_color }}">
                                        <i class="fa {{ $type->icon }}"></i> {{ $type->title }}
                                    </span>
                                </td>
                                <td class="py-4 px-6">
                                    <div class="flex flex-col">
                                        <span class="text-base font-semibold">{{ $type->created_at->diffForHumans() }}</span>
                                        <span class="font-normal text-gray-500">{{ $type->created_at->format(__('Y-m-d g:i A')) }}</span>
                                    </div>
                                </td>
                                <td class="py-4 px-6">
                                    <button wire:click="updateType('{{ $type->id }}')" type="button" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">@lang('Edit priority')</button>
                                </td>
                            </tr>
                        @endforeach
                    @else
